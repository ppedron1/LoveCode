...
(53,'Miguel Chainsaw','miguelchain@gmail.com','$2a$10$SKzS1N2fvo9pWufhRJLE3Oa5zTOgNlQFENUfNqlyJuxX09hjoBlPa','Me gusta JavaScript, Java y estoy aperendiendo Python','root','inactivo','2026-05-26 08:29:09');
...
BEGIN
    ...
    ELSE
        -- 1. Limpiamos sus tecnologías asignadas
        DELETE FROM Usuarios_Tecnologias WHERE id_usuario = p_id_usuario;
        
        -- 2. Limpiamos sus likes dados o recibidos (esto limpia los matches en cascada por trigger)
        DELETE FROM Likes WHERE id_usuario_da = p_id_usuario OR id_usuario_recibe = p_id_usuario;
        
        -- 3. Cambiamos su estado a 'inactivo' en lugar de borrar el registro físicamente
        UPDATE Usuarios SET estado_usuario = 'inactivo' WHERE id_usuario = p_id_usuario;
...
BEGIN
    -- Solo permite encontrar y autenticar usuarios en estado activo
    SELECT id_usuario, nombre, password 
    FROM Usuarios 
    WHERE email = p_email AND estado_usuario = 'activo';
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
    -- Excluimos del listado completo a los usuarios inactivos
...
BEGIN
    -- Ocultamos a todos los usuarios inactivos de la visualización de perfiles
    SELECT id_usuario as id, nombre, email, ciudad, descripcion 
    FROM Usuarios 
...
-- Dump completed on 2026-05-26  9:22:34